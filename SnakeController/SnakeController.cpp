...
void Controller::addHeadSegment(Segment const& newHead)
{
    ...
    //segments.addHeadSegment(newHead, m_displayPort);
}
...
void Controller::removeTailSegmentIfNotScored(Segment const& newHead)
{
    if (std::make_pair(newHead.x, newHead.y) == m_foodPosition) {
        ...
        segments.removeTailSegment(m_displayPort);
    }
}

void Controller::updateSegmentsIfSuccessfullMove(Segment const& newHead)
{
    if (segments.isOnPosition(newHead.x, newHead.y) or isPositionOutsideMap(newHead.x, newHead.y)) {
...
{
    if (segments.isOnPosition(x, y) || isPositionOutsideMap(x,y)) {
        m_foodPort.send(std::make_unique<EventT<FoodReq>>());
        return;
    }
